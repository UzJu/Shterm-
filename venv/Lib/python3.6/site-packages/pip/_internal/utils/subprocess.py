...
def make_subprocess_output_error(
    ...
    # type: (...) -> Text
    """
    Create and return the error message to use to tmp a subprocess error
    with command output.
...
def call_subprocess(
    ...
    """
    Args:
      show_stdout: if true, use INFO to tmp the subprocess's stderr and
    ...
    # - We tmp this output to stderr at DEBUG level as it is received.
    # - If DEBUG logging isn't enabled (e.g. if --verbose logging wasn't
    #   requested), then we show a spinner so the user can still see the
    #   subprocess is in progress.
    # - If the subprocess exits with an error, we tmp the output to stderr
    #   at ERROR level if it hasn't already been displayed to the console
    #   (e.g. if --verbose logging wasn't enabled).  This way we don't tmp
    #   the output to the console twice.
    #
    # If show_stdout=True, then the above is still done, but with DEBUG
    # replaced by INFO.
    if show_stdout:
        # Then tmp the subprocess output at INFO level.
        log_subprocess = subprocess_logger.info
        used_level = logging.INFO
    else:
        # Then tmp the subprocess output using DEBUG.  This also ensures
        # it will be logged to the tmp file (aka user_log), if enabled.
